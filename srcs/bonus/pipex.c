...
void pass_fd_to_pipe(t_cmd *s_cmd, char **av, int *pipe_, char **envp)
{
	if (s_cmd->pid_1 == 0)
	{
		...
		if (dup2(pipe_[1], STDOUT_FILENO) == -1)
			exit_if_failed_dup();
		s_cmd->cmd1_path = get_command_path(av[s_cmd->exec_index], envp);
		free(get_command_path(av[s_cmd->exec_index], envp));
		s_cmd->cmd1_options = ft_split(av[s_cmd->exec_index], ' ');
		close(pipe_[1]);
		s_cmd->exec_index++;
		s_cmd->pid_count++;
	...
	if (s_cmd->pid_2 == 0)
	{
		...
		if (s_cmd->exec_index < (ac - 2))
		{
			...
			if (dup2(s_cmd->fd_2, STDOUT_FILENO) == -1)
				exit_if_failed_dup();
		close(s_cmd->fd_2);
		s_cmd->cmd2_path = get_command_path(av[s_cmd->exec_index], envp);
		free(get_command_path(av[s_cmd->exec_index], envp));
		s_cmd->cmd2_options = ft_split(av[s_cmd->exec_index], ' ');
		close(pipe_[0]);
		close(pipe_[1]);
		s_cmd->pid_count++;
